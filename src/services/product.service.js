...
class ProductService {
    async createProduct(productData) {
        ...
        if (allProducts && allProducts.length > 0) {
            const nums = allProducts.map(p => p.num || 0);
            num = Math.max(...nums) + 1;
        }
        const newProduct = {
            _id: identifier,
    ...
    async getByProductNum(num) {
        if (typeof num !== "number") throw new Error("El número de producto debe ser un número válido");
        const product = await productRepo.getByNum(num);
        if (!product) throw new Error("Producto no encontrado con ese número");
        return product;
    }
    ...
    async updateProduct(pid, updateData) {
        if (!pid || typeof updateData !== "object") throw new Error("Datos inválidos para actualizar el producto");
        const result = await productRepo.update(pid, updateData);
        if (!result || result.modifiedCount === 0) throw new Error("No se pudo actualizar el producto");
    ...
    async deleteProductByID(pid) {
        ...
        if (!result || result.deletedCount === 0) throw new Error("No se pudo eliminar el producto o no existe");
        return { status: "success", message: "Producto eliminado correctamente" };
    }

    async deleteProductByNum(num) {
        if (typeof num !== "number") throw new Error("Número de producto inválido");
